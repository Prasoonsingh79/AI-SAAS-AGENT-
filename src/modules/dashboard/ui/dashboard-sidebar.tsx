...
export const DashboardSidebar = () => {
  ...
  return (
    <div className="flex h-screen w-64 flex-col bg-gradient-to-b from-[#1e293b] via-[#1a2332] to-[#0f172a] border-r border-slate-700/50 shadow-2xl fixed left-0 top-0">
      {/* Sidebar Header */}
      ...
      {/* Divider */}
      <div className="mx-5 mb-4 h-px bg-gradient-to-r from-transparent via-slate-600/30 to-transparent" />

      {/* Main Navigation */}
      <div className="flex-1 overflow-auto px-3 space-y-6">
        {/* First Section */}
        <nav className="space-y-1">
          <p className="px-3 text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Menu</p>
        ...
        {/* Second Section */}
        <div>
          <div className="mx-3 mb-3 h-px bg-gradient-to-r from-transparent via-slate-600/30 to-transparent" />
          <p className="px-3 text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Settings</p>
          <nav className="space-y-1">
            {secondSection.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={cn("h-5 w-5 flex-shrink-0", isActive ? "text-amber-400" : "text-slate-500")} />
                  {item.label}
                </Link>
              );
            })}
      ...
      {/* Footer */}
      <footer className="border-t border-slate-700/50 bg-slate-900/50 px-4 py-4 mt-auto">
        <DashboardUserButton />
      </footer>
    </div>
  );
